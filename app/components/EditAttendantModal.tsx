...
export default function EditAttendantModal({ attendant, onClose, onSuccess }: EditAttendantModalProps) {
  ...
  useEffect(() => {
    if (attendant) {
      const workDaysArray = typeof attendant.workDays === 'string' 
        ? attendant.workDays.split(',').map(day => day.trim())
        : attendant.workDays || [];
      ...
      setFormData({
        ...
        cpf: attendant.cpf || '',
        positionId: attendant.positionId || attendant.position?.id || '',
        functionId: attendant.functionId || attendant.function?.id || '',
        departmentId: attendant.departmentId || attendant.department?.id || '',
        managerId: attendant.managerId || attendant.manager?.id || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || 'Erro ao atualizar atendente');
      }
    } catch (error) {
      setError('Erro ao atualizar atendente');
      ...
      <div className="bg-gray-900 text-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-700">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold">Editar Atendente</h2>
            <button
              onClick={onClose}
